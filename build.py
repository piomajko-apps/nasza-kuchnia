...
/* ---------- stan + nawigacja ---------- */
let stack=[{view:"home"}];
let ctx={ meal:null, selected:new Set(), results:[], offset:0 };
...
function home(){ stack=[{view:"home"}]; ctx.selected=new Set(); render(); window.scrollTo(0,0); }
...
function renderMeal(meal){
  ctx.meal=meal; ctx.selected=new Set();
  ...
  h+='<p class="lead">Zaznacz, co masz w domu (możesz kilka). Nie musisz nic zaznaczać — wtedy pokażę propozycje dnia.</p>';
  h+='<div class="selbar"><div class="selcount" id="selcount">Wybrane produkty: 0</div></div>';
...
function updateSelCount(){
  ...
  if(el) el.textContent="Wybrane produkty: "+ctx.selected.size;
}

/* ---------- dopasowanie + wyniki ---------- */
function rankRecipes(meal, selected){
  const list=RECIPES.filter(r=>r.m.includes(meal));
...
function showResults(){
  ctx.results=rankRecipes(ctx.meal, ctx.selected);
  ctx.offset=0;
  go("results");
}
function threeNow(){
...
function renderResults(){
  ...
  if(ctx.selected.size>0){
    h+='<p class="lead">Dopasowane do produktów: '+[...ctx.selected].join(", ")+'.</p>';
  }else{
...
document.body.addEventListener("click", (e)=>{
  ...
  if(a==="toggle-ing"){
    ...
    else { ctx.selected.add(ing); t.classList.add("on"); t.setAttribute("aria-pressed","true"); }
